    ...
    async function fetchUserData() {
      try {
        ...
        if (userData) {
          ...
          if (!bizVeriId) {
            setMessage("It's time to take your business to the next level! 🚀\n" + 
    ...
    <SafeAreaContainer>
      ...
      <View style={styles.content}>
        <View style={{ alignItems: "center" }}>
          {message.split('\n').map((line, index) => (
            <RegularText typography="H4" key={index} style={{ textAlign: "center", marginBottom: 10 }} >
              {line}
            </RegularText>
          ))}
        </View>
