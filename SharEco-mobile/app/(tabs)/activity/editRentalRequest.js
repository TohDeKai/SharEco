import {
  ...
  const editRentalRequest = () => {
    ...
    const [selectedLocation, setSelectedLocation] = useState(rental.collectionLocation ? rental.collectionLocation : null);
      ...
      async function fetchRentalData() {
        try {
          ...
          if (response.status === 200) {
            ...
            console.log("rentaldata.startdate", new Date(rentalData.startDate));
            console.log(rentalData.startDate);
          } else {
            // Shouldn't come here
      ...
      async function fetchListingData() {
        ...
        try {
          const unavailResponse = await axios.get(
            `http://${BASE_URL}:4000/api/v1/item/unavailability/${rental.itemId}`
          );
        ...
        try {
          const unavailResponse = await axios.get(
            `http://${BASE_URL}:4000/api/v1/item/unavailability/fullDay/${rental.itemId}`
          );
    ...
    console.log("--> RENTAL START DATE", rentalStart)

    const activeTab= listingItem.isHourly ? "Hourly" : "Daily";
    ...
    const [startDate, setStartDate] = useState(rental ? rentalStart: tomorrow());
    const [endDate, setEndDate] = useState(new Date(rental.endDate));
    const [startTime, setStartTime] = useState(new Date(rental.startDate));
    const [endTime, setEndTime] = useState(new Date(rental.endDate));
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const response = await axios.get(
            `http://${BASE_URL}:4000/api/v1/item/availability/${rental.itemId}/${formattedDate}`
          );
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          console.log("FormattedDate: ", formattedDate);
          const response = await axios.get(
            `http://${BASE_URL}:4000/api/v1/item/nextBooking/${rental.itemId}/${formattedDate}`
          );
          if (response.status === 200) {
    ...
    const newStatus = rental.status == "PENDING" ? rental.status : "UPDATED";
  
    const handleCreateRentalRequest = async (values) => {
      try {
        const reqData = {
          startDate: fullStartDate(activeTab) || rental.startDate,
          endDate: fullEndDate(activeTab) || rental.endDate,
          ...
          rentalFee: rentalCost(activeTab) || rental.rentalFee,          
          totalFee: totalCost(activeTab) || rental.totalFee,
          status: newStatus,
        };
    ...
    return (
      <SafeAreaContainer>
        <KeyboardAvoidingView behavior="padding" style={styles.container}>
          <Header title="Edit Rental Request" action="close" onPress={handleBack} />
